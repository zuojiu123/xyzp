    ...
    <el-dialog :visible.sync="dialogFormVisible" :title="type === 'add'? '新增数据' : '修改数据'">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="70px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item prop="role" label="角色">
          <el-select v-model="temp.role" class="filter-item" placeholder="请选择角色" style="width: 100%;">
            <el-option v-for="item in availableRoles" :key="item.enumCode" :label="item.msg" :value="item.enumCode">
              <span style="float: left">{{ item.msg }}</span>
              <span style="float: right; color: #8492a6; font-size: 13px">{{ item.enumCode }}</span>
            </el-option>
          </el-select>
        </el-form-item>
      </el-form>
...
export default {
  ...
  computed: {
    ...
    availableRoles() {
      return [
        { enumCode: 'Normal_User', msg: '普通用户' },
        { enumCode: 'Admin', msg: '管理员' },
        { enumCode: 'Enterprise_User', msg: '企业用户' }
      ]
    }
  },
