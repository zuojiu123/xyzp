...
public class ResumeController {
    ...
    public Result getUserResumeList(@PathVariable("pageNum") Integer pageNum,
        ...
        try {
            String userId = GetTokenInfoUtil.getUsername();
            System.out.println("获取简历列表 - 用户ID: " + userId);
            
            if ("noLogin".equals(userId) || userId == null || userId.trim().isEmpty()) {
                return Result.failed("请先登录后查看简历");
            }
            ...
            java.util.List<UserResume> resumeList = userResumeDao.queryAllByLimit(queryParam);
            System.out.println("查询到的简历数量: " + (resumeList != null ? resumeList.size() : 0));
            
            PageInfo<UserResume> pageInfo = new PageInfo<>(resumeList);
    ...
    public Result getAllResumeList(@PathVariable("pageNum") Integer pageNum,
        ...
        try {
            ...
            // 使用分页查询所有简历
            PageHelper.startPage(pageNum, pageSize);
            java.util.List<UserResume> resumeList = userResumeDao.queryAllByLimit(new UserResume());
            PageInfo<UserResume> pageInfo = new PageInfo<>(resumeList);
