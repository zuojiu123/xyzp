...
public class EmploymentUserController extends SuperController {
    ...
    @PostMapping("/admin/{pageNum}/{pageSize}")
    public Result queryByPageAdmin(@RequestBody EmploymentUserParam employmentUser,
                              @PathVariable("pageNum") Integer pageNum,
                              @PathVariable("pageSize") Integer pageSize) {
        return success(this.employmentUserService.queryByPageAdmin(employmentUser, pageNum, pageSize));
    }
    ...
    public Result getByUser(@PathVariable("pageNum") Integer pageNum,
        ...
        try {
            ...
            System.out.println("查询申请记录 - 用户名: " + username);
            
            if ("noLogin".equals(username) || username == null || username.trim().isEmpty()) {
                return Result.failed("请先登录后查看申请记录");
            }
            
            PageHelper.startPage(pageNum, pageSize);
            List<EmploymentUserModel> employmentUserModels = employmentUserService.getByUser(username);
            System.out.println("查询到的申请记录数量: " + (employmentUserModels != null ? employmentUserModels.size() : 0));
            
            PageInfo<EmploymentUserModel> pageInfo = new PageInfo<>(employmentUserModels);
            System.out.println("分页信息 - 总数: " + pageInfo.getTotal() + ", 当前页数据: " + pageInfo.getList().size());
            
            return Result.ok(pageInfo);
        } catch (Exception e) {
            e.printStackTrace();
            return Result.failed("获取申请记录失败: " + e.getMessage());
        }
    }
